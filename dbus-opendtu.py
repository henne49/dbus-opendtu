#!/usr/bin/env python
"""Python driver/script to read a DTU and update DBUS"""
import logging
import os
import platform
import sys
...
def get_ahoy_field_by_name(meter_data, actual_inverter, fieldname):
    """Get Ahoy Data from JSON by name"""
...
class DbusService:
    """Main Service to register Inverter/Meter into dbus"""
    __metaclass__ = PvInverterRegistry
    _registry = []
    _meter_data = None
    _test_meter_data = None

    def __init__(self, servicename, paths, actual_inverter, istemplate=False):
        productname='OpenDTU'
        ...
        ) if 'DBUS_SESSION_BUS_ADDRESS' in os.environ else dbus.SystemBus(private=True)
        ...
        self._dbusservice.add_path(
            '/CustomName', self._get_name(self.pvinverternumber))
        ...
        self._dbusservice.add_path(
            '/Serial', self._get_serial(self.pvinverternumber))
        ...
        gobject.timeout_add(self._get_sign_of_lifeinterval()
                            * 60*1000, self._sign_of_life)

    # read config file

    def _read_config_dtu(self, actual_inverter):
        """Read JSON Config of Ahoy or OpenDTU and fill variales"""
        config = self._get_config()
        self.pvinverternumber = actual_inverter
        self.dtuvariant = str(config['DEFAULT']['DTU'])
        self.deviceinstance = int(
            config[f"INVERTER{self.pvinverternumber}"]['DeviceInstance'])
        try:
            self.acposition = int(
        ...
        self.pollinginterval = int(
            config['DEFAULT']['ESP8266PollingIntervall'])
        self.meter_data = 0

    def _read_config_template(self, template_number):
        """Read Template Config from config.ini and fill variales"""
        config = self._get_config()
        ...
        self.serial = str(
            config[f"TEMPLATE{template_number}"]['CUST_SN'])
        self.pollinginterval = int(
            config[f"TEMPLATE{template_number}"]['CUST_POLLING'])
        ...
        self.deviceinstance = int(
            config[f"TEMPLATE{template_number}"]['DeviceInstance'])
        self.customname = config[f"TEMPLATE{template_number}"]['Name']
        self.acposition = int(
            config[f"TEMPLATE{template_number}"]['AcPosition'])
        self.signofliveinterval = config['DEFAULT']['SignOfLifeLog']
        self.useyieldday = int(config['DEFAULT']['useYieldDay'])
        self.pvinverterphase = str(
            config[f"TEMPLATE{template_number}"]['Phase'])
        try:
            self.max_age_ts = int(config['DEFAULT']['MagAgeTsLastSuccess'])
        except:
            self.max_age_ts = 600
    ...
    def _get_serial(self, pvinverternumber):
        ...
        if self.dtuvariant == 'ahoy':
            ...
            if not meter_data['inverters'][pvinverternumber]['serial']:
                raise ValueError(
                    "Response does not contain serial attribute try name")
    ...
    def _get_polling_interval(self):
        """Get Polling Interval from config.ini file and limit ESP8266 polling"""
        meter_data = self._get_data()
        dtuvariant= self._get_dtu_variant()
        if self.dtuvariant == 'ahoy':
            # Check for ESP8266 and limit polling
            try:
                esptype = meter_data['generic']['esp_type']
            except:
                esptype = meter_data['system']['esp_type']
            if esptype == 'ESP8266':
    ...
    def _sign_of_life(self):
        """Report a Sign of Life in Log file every defined interval"""
        logging.info("--- Start: sign of life ---")
        logging.info("Last inverter #%d _update() call: %s",
                     self.pvinverternumber, self._last_update)
        logging.info("Last inverter #%d '/Ac/Power': %s",
    ...
    def _update(self):
        try:
            ...
            if self.is_data_up_to_date():
                ...
                logging.debug("Inverter #%d Power (/Ac/Power): %s",
                              self.pvinverternumber, power)
                logging.debug("Inverter #%d Energy (/Ac/Energy/Forward): %s",
    ...
    def get_values_for_inverter(self):
        """Get Values for different inverter and return normalized"""
        meter_data = self._get_data()
        (power, pvyield, current, voltage) = (None, None, None, None)
        pvinverter = self.pvinverternumber

        if self.dtuvariant == 'ahoy':
            power = get_ahoy_field_by_name(
                meter_data, pvinverter, 'P_AC')
            if self.useyieldday:
                pvyield = get_ahoy_field_by_name(
                    meter_data, pvinverter, 'YieldDay') / 1000
            else:
                pvyield = get_ahoy_field_by_name(
                    meter_data, pvinverter, 'YieldTotal')
            voltage = get_ahoy_field_by_name(
                meter_data, pvinverter, 'U_AC')
            current = get_ahoy_field_by_name(
                meter_data, pvinverter, 'I_AC')

        elif self.dtuvariant == 'opendtu':
            producing = is_true(
                meter_data['inverters'][pvinverter]['producing'])
            if producing:
            ...
            if self.useyieldday:
                ...
                pvyield = meter_data['inverters'][pvinverter]['AC']['0']['YieldTotal']['v']
            voltage = meter_data['inverters'][pvinverter]['AC']['0']['Voltage']['v']
            if producing:
                current = meter_data['inverters'][pvinverter]['AC']['0']['Current']['v']
            else:
                current = 0

        elif self.dtuvariant == 'template':
            # logging.debug("JSON data: %s" % meter_data)
            power = float(get_nested(meter_data, self.custpower)
                          * float(self.custpower_factor))
            pvyield = float(get_nested(meter_data, self.custtotal)
                            * float(self.custtotal_factor))
...
def main():
    ...
    try:
        ...
        # Have a mainloop, so we can send/
        # receive asynchronous calls to and from dbus
...
OPENDTU_TEST_DATA_STR = """{"inverters":[{"serial":"112181311701","name":"Holzpalast Süd",
    "data_age":11559,"reachable":false,"producing":false,"limit_relative":100,
    "limit_absolute":350,"0":{"Power":{"v":1,"u":"W"},"Voltage":{"v":235.1999969,"u":"V"},
    "Current":{"v":1,"u":"A"},"Power DC":{"v":1.200000048,"u":"W"},
    "YieldDay":{"v":482,"u":"Wh"},"YieldTotal":{"v":111.3209991,"u":"kWh"},
    "Frequency":{"v":49.99000168,"u":"Hz"},"Temperature":{"v":21.60000038,"u":"°C"},
    "PowerFactor":{"v":0,"u":"%"},"ReactivePower":{"v":0,"u":"var"},"Efficiency":{"v":0,"u":"%"}},
    "1":{"Power":{"v":1.200000048,"u":"W"},"Voltage":{"v":24.10000038,"u":"V"},
    "Current":{"v":0.050000001,"u":"A"},"YieldDay":{"v":482,"u":"Wh"},
    "YieldTotal":{"v":111.3209991,"u":"kWh"},"Irradiation":{"v":0.292682916,"u":"%"}},
    "events":3},{"serial":"112180719948","name":"Holzpalast Ost Links","data_age":11678,
    "reachable":false,"producing":false,"limit_relative":100,"limit_absolute":300,
    "0":{"Power":{"v":0,"u":"W"},"Voltage":{"v":235.6000061,"u":"V"},"Current":{"v":0,"u":"A"},
    "Power DC":{"v":1,"u":"W"},"YieldDay":{"v":351,"u":"Wh"},
    "YieldTotal":{"v":0.843999982,"u":"kWh"},"Frequency":{"v":49.97000122,"u":"Hz"},
    "Temperature":{"v":21.70000076,"u":"°C"},"PowerFactor":{"v":0,"u":"%"},
    "ReactivePower":{"v":0,"u":"var"},"Efficiency":{"v":0,"u":"%"}},"1":{"Power":{"v":1,"u":"W"},
    "Voltage":{"v":19.70000076,"u":"V"},"Current":{"v":0.050000001,"u":"A"},
    "YieldDay":{"v":351,"u":"Wh"},"YieldTotal":{"v":0.843999982,"u":"kWh"},
    "Irradiation":{"v":0.289855093,"u":"%"}},"events":3},
    {"serial":"114181304338","name":"Mülltonnen","data_age":11377,
    "reachable":false,"producing":false,"limit_relative":100,"limit_absolute":600,
    "0":{"Power":{"v":0,"u":"W"},"Voltage":{"v":234.5,"u":"V"},"Current":{"v":0,"u":"A"},
    "Power DC":{"v":0.700000048,"u":"W"},"YieldDay":{"v":828,"u":"Wh"},"YieldTotal":{"v":5.251999855,"u":"kWh"},
    "Frequency":{"v":49.99000168,"u":"Hz"},"Temperature":{"v":22.39999962,"u":"°C"},"PowerFactor":{"v":0,"u":"%"},
    "ReactivePower":{"v":0,"u":"var"},"Efficiency":{"v":0,"u":"%"}},"1":{"Power":{"v":0.300000012,"u":"W"},
    "Voltage":{"v":18.60000038,"u":"V"},"Current":{"v":0.02,"u":"A"},"YieldDay":{"v":398,"u":"Wh"},
    "YieldTotal":{"v":2.50999999,"u":"kWh"},"Irradiation":{"v":0.086956523,"u":"%"}},"2":{"Power":{"v":0.400000006,"u":"W"},
    "Voltage":{"v":18.60000038,"u":"V"},"Current":{"v":0.02,"u":"A"},"YieldDay":{"v":430,"u":"Wh"},
    "YieldTotal":{"v":2.742000103,"u":"kWh"},"Irradiation":{"v":0.115942039,"u":"%"}},
    "events":3}]}"""
AHOY_TEST_DATA_STR = """{"menu":{"name":["Live","Serial / Control","Settings","-","REST API","-",
    "Update","System","-","Documentation"],"link":["/live","/serial","/setup",null,"/api",null,"/update",
    "/system",null,"https://ahoydtu.de"],"trgt":[null,null,null,null,"_blank",null,null,null,null,"_blank"]},
    "generic":{"version":"0.5.70","build":"d8e255d","wifi_rssi":-72,"ts_uptime":1602,"esp_type":"ESP8266"},
    "inverter":[{"enabled":true,"name":"hoymiles1","channels":1,"power_limit_read":100,"last_alarm":"Inverter start",
    "ts_last_success":1675243378,"ch":[[234.9,0.1,22.5,50.04,1,7.5,96.802,16,23.6,95.339,0],[33,0.71,23.6,16,96.802,6.743]],
    "ch_names":["AC","einzel"]},{"enabled":true,"name":"hoymiles2","channels":4,
    "power_limit_read":100,"last_alarm":"Inverter start","ts_last_success":1675243373,
    "ch":[[234.4,0.37,87.2,50.03,0.971,5.8,11.338,68,91.7,95.093,21.5],[34,0.76,26,19,3.554,6.933],
    [34,0.01,0.5,0,0.071,0],[33.8,0.95,32.1,24,3.581,8.56],[33.8,0.98,33.1,25,4.132,8.827]],
    "ch_names":["AC","M1","no","M3","M4"]}],"refresh_interval":5,"ch0_fld_units":["V","A","W","Hz","","°C","kWh","Wh","W","%","var"],
    "ch0_fld_names":["U_AC","I_AC","P_AC","F_AC","PF_AC","Temp","YieldTotal","YieldDay","P_DC","Efficiency","Q_AC"],
    "fld_units":["V","A","W","Wh","kWh","%"],"fld_names":["U_DC","I_DC","P_DC","YieldDay",
    "YieldTotal","Irradiation"]}"""

def test_opendtu_reachable(test_service):
    """Test if OpenDTU is reachable"""
    test_service.set_dtu_variant('opendtu')
    test_data = json.loads(OPENDTU_TEST_DATA_STR)

    test_service.set_test_data(test_data)
    assert test_service.is_data_up_to_date() is False

    test_data = json.loads(OPENDTU_TEST_DATA_STR.replace(
        '"reachable":false', '"reachable":"1"'))
    test_service.set_test_data(test_data)
    assert test_service.is_data_up_to_date() is True
...
def test_ahoy_timestamp(test_service):
    ...
    test_data = json.loads(AHOY_TEST_DATA_STR.replace(
...
def run_tests():
    """Execute Tests"""
    test_service = DbusService(
        servicename='testing', paths='dummy', actual_inverter=0)
